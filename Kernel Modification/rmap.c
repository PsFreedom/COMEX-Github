		     		...
		     		unsigned long old_address, pte_t *old_ptePointer)
{
	...
	mmun_start  = old_address & PAGE_MASK;
	mmun_end    = mmun_start + PAGE_SIZE;
	mmu_notifier_invalidate_range_start(old_mm, mmun_start, mmun_end);	
	
	clear_page_mlock(old_page);	
	flush_cache_page(old_vma, old_address, page_to_pfn(old_page));
	pteval = ptep_clear_flush(old_vma, old_address, old_ptePointer);
