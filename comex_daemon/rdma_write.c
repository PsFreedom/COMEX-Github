...
#define RDMA_STRING 	"Hello World!"
...
do_poll(struct rdma_cb *cb) {
	...
	while ((err = ibv_poll_cq(cb->cq, 1, &wc)) == 1) {
		...
		case IBV_WC_RDMA_WRITE:
			fprintf(stdout, "RDMA Write with Immediate completed successfully.\n");
			break;
 ...
 * 	 	 */ 
	...
	cb->rdma_buf_len = strlen(RDMA_STRING)+1;	/* +1 for NULL Char */
 ...
 * 	 	 */
	...
	/*
 * 	 * Zero out the rdma buffer.  This will get written by the client.
 * 	 	 */
 ...
 * 	 	 */
	...
	fprintf(stdout, "Sending RDMA Buffer information: addr: 0x%llx, length: %d, rkey: 0x%x\n",
 ...
 * 	 	 */
	...
	/*
 * 	 * Now that the client has the RDMA Buffer we need to wait until
 * 	 	 * they RDMA the buffer and notify us with an RDMA Write with IMM.
 * 	 	 	 * This means we sit in the completion handler again.
 * 	 	 	 	 */
	err = do_completion(cb);
	if (err) {
		goto disconnect;
 ...
 * 	 	 */
	fprintf(stdout, "Client RDMA'ed the following data into my memory: %s\n", cb->rdma_buffer);

	/*
 * 	 * Perform another Send to the client, which lets the client know we have
 * 	 	 * processed the data from the RDMA write.  Send buffer contents are
 * 	 	 	 * irrelevant.
 * 	 	 	 	 */
...
/////////////////////////////////////////////////////////////////////////////////////
	/*
 * 	 * Wait for the server to send the rdma buffer information
 * 	 	 */
	fprintf(stdout, "Waiting for server rdma buffer info\n");
	err = do_completion(cb);
	if (err) {
		goto disconnect;
	}
	fprintf(stdout, "Received RDMA Buffer information: addr: 0x%llx, length: %d, rkey: 0x%x\n",
			(unsigned long long)cb->remote_buffer.addr, cb->remote_buffer.length, cb->remote_buffer.rkey);

	/*
 * 	 * Perform the RDMA Write with Imediate
 * 	 	 */
	strncpy(cb->rdma_buffer, RDMA_STRING, cb->rdma_buf_len);
	fprintf(stdout, "Issuing an RDMA Write with immediate to the server\n");
	err = do_send(cb, IBV_WR_RDMA_WRITE_WITH_IMM);
	if (err) {
		goto disconnect;
	}

	/*
 * 	 * Check the completion of the write with immediate data
 * 	 	 */
	err = do_completion(cb);
	if (err) {
		goto disconnect;
 ...
 * 	 	 */
	if (argc == 1) {
		...
		fprintf(stdout, "Running Client, Server IP %s\n", argv[1]);
